...
/*----------------------------------------------------------------------
    Função adaptada do algoritmo trivial de busca de palavra
    (aula 24), com a única diferença que retorna TRUE na primeira
    comparação de sucesso, em oposição a retornar o número de vezes 
    que ocorre a palavra no texto.
...
achePalavra(unsigned char *pal, int tPal, unsigned char *texto, int tTex)
{
  ...
  {
    r = 0;
    while(r < tPal && pal[1+r] == texto[k+r])
      r+=1;
    if (r == tPal){
      printf("\nDeu TRUE");
      return TRUE;
