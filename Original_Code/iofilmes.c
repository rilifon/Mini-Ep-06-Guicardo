...
mostreListaFilmes(ListaFilmes *lst)
{
	...
	i = 0; /* inicializa os dois */
	p = lst->cab->prox;

	/* percorre a lista até "dar a volta" */
...
mostreMelhoresFilmes(ListaFilmes *lst)
{
	...
	float  x; /* nota maxima */
	int    v; /* numero minimo de votos */
	int    i; /* contador */

	/* inicializa o ponteiro e o contador */

	p = lst->cab->prox;
	i = 0;

	/* pega da entrada o valor de N, X e V */
	...
	/* percorre a lista até que a nota de um filme seja menor/igual a x */

	while(p->nota > x)
		p = p->prox;

	/* mostra os n filmes com pelo menos v votos (ou menos caso a lista acabe antes) */
	...
	while( i < n && p != lst->cab )
	{
		...
		p = p->prox; /* "incremento" no ponteiro */
	}

	printf("\nEsses sao os %d melhores filmes com nota menor que %2.1f e pelo menos %d votos",
		   i, x, v);		
...
mostrePioresFilmes(ListaFilmes *lst)
{
	...
	/* percorre a lista até que a nota de um filme seja maior/igual a x */
	...
	printf("\nEsses sao os %d piores filmes com nota maior que %2.1f e pelo menos %d votos",
		   i, x, v);		
